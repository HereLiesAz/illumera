...
text = p.read_text()
text = text.replace('import com.hereliesaz.illumera.remote_input.DebridPairingServerManager\n', '')
text = text.replace('    var pairingServerInfo by remember { mutableStateOf<ServerInfo?>(null) }\n    var pairingQrBitmap by remember { mutableStateOf<Bitmap?>(null) }\n', '')
text = text.replace('    val pairingServerManager = remember { DebridPairingServerManager() }\n', '')
text = text.replace('''    LaunchedEffect(deviceFormFactor, connectedProvider, selectedProvider) {
...
''', '')
old = '''                        if (deviceFormFactor == DeviceFormFactor.TV) {
...
'''
new = '''                        if (deviceFormFactor == DeviceFormFactor.TV) {
...
'''
if old not in text:
    raise SystemExit('TV debrid block not found')
